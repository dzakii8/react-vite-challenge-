...
describe('get /lodgings', () => {
  test('should read all lodgings', async () => {
    let { status, body } = await request(app).get('/lodgings').set('Authorization', `Bearer ${access_token}`)
    expect(status).toBe(200)
  })
  test('should fail if not logged', async () => {
    let { status, body } = await request(app).post('/lodgings')
  ...
  test('should fail if token not valid', async () => {
    let { status, body } = await request(app).post('/lodgings').set('Authorization', `Bearer fasfsafasf`)
    expect(status).toBe(401)
    expect(body).toEqual({
      message: "you are not authorized"
    })
