...
describe('get /lodgings', () => {
  test('should read lodgings with id = 1', async () => {
    let { status, body } = await request(app).get('/lodgings/1').set('Authorization', `Bearer ${access_token}`)
    expect(status).toBe(200)
    expect(body).toMatchObject(lodging)
  })
  test('should fail if not logged', async () => {
    let { status, body } = await request(app).get('/lodgings/1')
  ...
  test('should fail if token not valid', async () => {
    let { status, body } = await request(app).get('/lodgings/1').set('Authorization', `Bearer fasfsafasf`)
    expect(status).toBe(401)
    expect(body).toEqual({
      message: "you are not authorized"
    })
