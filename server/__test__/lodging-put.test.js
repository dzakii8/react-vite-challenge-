...
describe('post /lodgings', () => {
  test('should update lodgings', async () => {
    let lodging = {
      "name": "Hotel fafasf",
      "facility": "gym, spa",
      "roomCapacity": 2,
  ...
  test('should fail if not logged', async () => {
    ...
    let { status, body } = await request(app).put('/lodgings/1').send(lodging)
  ...
  test('should fail if token is not valid ', async () => {
    ...
    let { status, body } = await request(app).put('/lodgings/100').set('Authorization', `Bearer sfasfasfsaf`).send(lodging)
    expect(status).toBe(401)
  ...
  test('should fail if id Lodging not found ', async () => {
    ...
    let { status, body } = await request(app).put('/lodgings/100').set('Authorization', `Bearer ${access_token}`).send(lodging)
  ...
  test.only('should fail if staff edit not his own', async () => {
    let lodging = {
      "name": "Hotel fafasf",
      "facility": "gym, spa",
      "roomCapacity": 2,
    ...
    let { status, body } = await request(app).put('/lodgings/1').set('Authorization', `Bearer ${access_token_staff}`).send(lodging)
