...
describe('get /type', () => {
  test('should read all type', async () => {
    let { status, body } = await request(app).get('/types').set('Authorization', `Bearer ${access_token}`)
    expect(status).toBe(200)
  })
  test('should fail if not logged', async () => {
    let { status, body } = await request(app).get('/types')
  ...
  test('should fail if token not valid', async () => {
    let { status, body } = await request(app).get('/types').set('Authorization', `Bearer fasfsafasf`)
    expect(status).toBe(401)
    expect(body).toEqual({
      message: "you are not authorized"
    })
